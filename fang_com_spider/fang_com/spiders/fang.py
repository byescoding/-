...
from fang_com.items import FangComItem
class FangSpider(RedisSpider):
    ...
    #     '''这里提取新房跟二手房的url链接然后请求'''
        ...
        #进入租房信息页面
        #
        zf_url = response.xpath('//a[text()="找租房"]/@href').get()
        zf_url = response.urljoin(zf_url)
        if zf_url is None:
            return
        yield scrapy.Request(url=zf_url, callback=self.zf, meta={"item": copy.deepcopy(item)})
    ...
    #         #评论人数
    #         item["comment_num"]=li.xpath('.//span[@class="value_num"]/text()').get()
    #
    #         # 居室跟平米
    ...
    #         # #评论数
    #         # item['']=li.xpath('.//span[@class="value_num"]/text()')
    #
    #         yield item
    #
    #     # 翻页
    ...
    #         #优点
    #         advantage=dl.xpath('.//p[@class="clearfix label"]//text()').getall()
    #         advantage = [re.sub(r'\s|/|－|\|', '', i) for i in advantage]  # 将\n\t等字符替换为空字符
    #         item["advantage"] = [i for i in advantage if len(i) > 0]  # 去掉空字符
    #         # 总价格
    ...
    def zf(self,response):
        ...
        for dl in dl_list:
            # 租房标题
            item["title"] = dl.xpath('.//p[@class="title"]/a/@title').get()
            # 居室跟平米
            jushi = dl.xpath('.//p[@class="font15 mt12 bold"]//text()').getall()
            jushi = [re.sub(r'\s|/|－|\|', '', i) for i in jushi]  # 将\n\t等字符替换为空字符
            item["type"] = [i for i in jushi if len(i) > 0]  # 去掉空字符

            #地址
            add_shop = dl.xpath('.//p[@class="gray6 mt12"]//text()').getall()
            add_shop = [re.sub(r'\s-', '', i) for i in add_shop]  # 将\n\t等字符替换为空字符

            item["address"] = [i for i in add_shop if len(i) > 0]  # 去掉空字符

            #地铁站的距离
            item["distance"]= dl.xpath('.//p/span[@class="note subInfor"]/text()').getall()

            # 价格
            item["price"] =dl.xpath('.//p[@class="mt5 alingC"]/span/text()').get()

            #优点
            item["advantage"]=dl.xpath('.//p[@class="mt12"]/span/text()').getall()
            yield item
        ...
        if next_url is not None and next_url != "javascript:void(0)":
            # 在这里组合成完整链接，因为这里是已经过滤的了，能节省时间
            next_url = response.urljoin(next_url)
            yield scrapy.Request(url=next_url, callback=self.zf, meta={"item": copy.deepcopy(item)})
        else:
            # 这里因为有些页面没有下一页，所以只能获取全部的url，而且scrapy不会请求重复的url，这里也不用担心
            all_url = list(response.xpath('//div[@class="fanye"]/a/@href').getall())  # 使用集合去下重保  //*[@id="listBox"]/div[4]/a[1]
            print(all_url)
            print(list(response.xpath('//div[@class="fanye"]/a/text()').getall()))
            for url in all_url:
                if url != "javascript:void(0)" or url is not None:
                    next_url = response.urljoin(url)
                    yield scrapy.Request(url=next_url, callback=self.zf, meta={"item": copy.deepcopy(item)})
